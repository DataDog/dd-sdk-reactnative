...
export default class MainScreen extends Component<any, MainScreenState> {
    consentModal: RefObject<ConsentModal | null>;
    ...
    axiosVehicle() {
        return axios
            .request({ method: 'get', url: '/api/vehicle/random_vehicle' })
            ...
            <View style={styles.defaultScreen}>
                ...
                <View style={{ marginTop: 40, alignItems: 'center' }}>
                    <Button
                        ...
                        onPress={() =>
                            this.setTrackingConsentModalVisible(true)
                        }
                    />
                    <ConsentModal
                    ...
                    <Text style={{ marginTop: 20 }}>
                        {this.state.callDatadogButtonAction}
                    </Text>
                    <Button
                        title="Fetch Datadog Logs"
                        accessibilityLabel="call_datadog_button"
                        onPress={() => {
                            this.fetchDatadogLogs()
                                .then(response => {
                                    console.log(response);
                                    this.setState({
                                        callDatadogButtonAction:
                                            'Datadog logs retrieved'
                                    } as MainScreenState);
                                })
                                .catch((error: any) => {
                                    this.setState({
                                        callDatadogButtonAction:
                                            'Unable to call Datadog'
                                    } as MainScreenState);
                                    console.log(error);
                                });
                        }}
                    ...
                    />
                    <Text style={{ marginTop: 20 }}>
                        {this.state.resultTouchableOpacityAction}
                    </Text>
                    <TouchableOpacity
                        ...
                        onPress={() => {
                            this.xhrRestaurant()
                                .then((json: any) => {
                                    ...
                                    this.setState({
                                        resultTouchableOpacityAction: msg
                                    } as MainScreenState);
                                    console.log(
                                        `Fetched restaurant:${json.name}`
                                    );
                                })
                                .catch(error => {
                    ...
                    <Text style={{ marginTop: 20 }}>
                        {this.state.resultTouchableWithoutFeedback}
                    </Text>
                    <TouchableWithoutFeedback
                        ...
                        onPress={() => {
                            this.axiosVehicle()
                                .then(json => {
                                    ...
                                    this.setState({
                                        resultTouchableWithoutFeedback: msg
                                    } as MainScreenState);
                                    console.log(
                                        `Fetched vehicle:${json.license_plate}`
                                    );
                                })
                                .catch((error: any) => {
                    ...
                    <Text style={{ marginTop: 20 }}>
                        {this.state.resultTouchableNativeFeedback}
                    </Text>
                    <TouchableNativeFeedback
